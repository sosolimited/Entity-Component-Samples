...
struct Transform : public HierarchyComponentT<Transform> {
  ...
  Transform(entityx::Entity entity, const ci::vec3 &position,
            const ci::vec3 &scale = ci::vec3(1),
            const ci::vec3 &pivot = ci::vec3(0),
            const ci::quat &orientation = ci::quat())
      : HierarchyComponentT(entity), position(position), scale(scale),
        pivot(pivot), orientation(orientation) {}
  ...
  ci::vec3 worldPoint() const {
    return ci::vec3(worldTransform() * ci::vec4(0, 0, 0, 1));
  }
  ...
  ci::mat4 calcLocalTransform() const {
    return glm::translate(position + pivot) * glm::toMat4(orientation) *
           glm::scale(scale) * glm::translate(-pivot / scale);
  }
...
inline Transform::Handle makeHierarchy(entityx::Entity a) {
  return a.has_component<Transform>() ? a.component<Transform>()
                                      : a.assign<Transform>(a);
}
...
void attachChildren(Transform::Handle root, entityx::Entity child1,
                    entityx::Entity child2, Children... children) {
